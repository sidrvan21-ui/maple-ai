...
Parts 0–9 built the cycle. After that we added: **rails** around the LLM, a **Maple take** instead of a class card, **SSO + audit**, **Docker files**, and a **Cloudflare share link**. None of that changes the USP: cycle + agentic RAG.
...
| `backend/app/agents/runner.py` `fill_maple_take` | Second LLM: stake, judgment, pushback, what Sign commits. Not JTBD textbook. |
| `GatePack.maple_take` | Stored on the pack. Old `pm_take` JSON still loads. |
...
| `backend/app/onboard.py` + `ingest.py` + `/onboard` | Company dump: any files or a zip. Convert to `.md` under `data/products/{slug}/`. Nine-room sort guide. Porter sample stays the default. |
...
Order: empty OpenAI key → load `.env` → rails → Maple take tab → SSO/audit → Docker files → `cloudflared tunnel --url http://127.0.0.1:8000`.
...
**Lesson tab?** Replaced by Maple take. Catalog `lesson` still exists on the spec; the UI does not teach frameworks.
...
1. Local: `backend` venv → uvicorn :8000 → login → Run → Citations → Maple take → Sign.  
